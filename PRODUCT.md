...
1. Typography first — the type system carries the identity; nothing competes with it.
2. Whitespace is content — generous, deliberate spacing over density.
3. Motion supports understanding — reveals, staggers, and draws explain structure; nothing loops or floats without purpose.
4. Every element must improve readability, hierarchy, storytelling, or usability — otherwise remove it.
5. The story is the product — sections exist to answer "why hire John Karlo?", not to fill space.

## Accessibility & Inclusion

- Honor `prefers-reduced-motion` everywhere: entrance choreography, scroll-linked transforms, smooth scrolling, and infinite loops all degrade to static/instant alternatives.
- Body text stays ≥4.5:1 contrast on the #F8F8F8 ground (neutral-600 or darker for prose).
- Semantic headings per page, aria labels on icon-only controls, keyboard-reachable menus (Escape closes the mobile menu).
